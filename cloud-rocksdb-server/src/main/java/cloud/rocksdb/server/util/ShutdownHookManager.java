...
/**
 * The shutdown hook manager enables running shutdown hook in a deterministic order, higher priority first.
 *
 * @author jie.huang
 *         Date: 15/10/20
 *         Time: 下午1:20
...
public class ShutdownHookManager {
                        ...
                        try {
                            for (Runnable hook : shutdownHooks) {
                                try {
                                    ...
                                    System.out.println("ShutdownHook '" + hook.getClass().getSimpleName() + "' failed, " + ex.toString());
                                    ex.printStackTrace();
                                }
                            }
                        } finally {
    ...
    private void addShutdownHook(Runnable shutdownHook, Priority priority) {
        if (shutdownHook == null) {
            throw new RuntimeException("shutdownHook cannot be NULL");
        }
        if (shutdownInProgress.get()) {
            throw new RuntimeException("Shutdown in progress, cannot add a shutdownHook");
        }
